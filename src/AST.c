	...
	while (stm) {
		...
		case ST_FUNC:
			...
			for (size_t i = 0; i < stm->func.param_count; i++) {
				printf( "\t\textern: %s, intern: %s, data_type: ",
					stm->func.parameters[i].extern_id->symbol.data,
					stm->func.parameters[i].intern_id->symbol.data
				);
				print_datatype( stm->func.parameters[i].type );
				printf("\n" );
			}
			printf("\t} \n");
			// todo print_exp(stm->exp.exp);
			printf("\treturn_type: ");
			print_datatype( stm->func.return_type );
		...
		case ST_VAR:
			...
			if (stm->var.id_prefix.func_id) {
				printf(
					"\tprefix_id. $%s%%%zu%%%s\n",
					stm->var.id_prefix.func_id->symbol.data,
					stm->var.id_prefix.block_counter,
					stm->var.id->symbol.data
				);
			} else {
				printf(
			...
			printf("\tdata_type: ");
			print_datatype( stm->var.data_type );
			printf("\n" );
...
void print_statement(Statement* stm, size_t tabs) {
    ...
    switch (stm->type) {
        ...
        case ST_FUNC:
            ...
            printf("%s\tparameters {\n", tab.data);
            for (size_t i = 0; i < stm->func.param_count; i++) {
                printf("%s\t\textern: %s, intern: %s, data_type: ", tab.data, stm->func.parameters[i].extern_id->symbol.data, stm->func.parameters[i].intern_id->symbol.data);
